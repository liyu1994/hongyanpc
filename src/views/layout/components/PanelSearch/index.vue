<template>
  ...
  >
    ...
    >
      <svg-icon
        icon-class="logo"
        class-name="card-panel-icon panel-search__logo"
      />
      <el-autocomplete
        ref="input"
        v-model="searchText"
        ...
        :placeholder="getLocale('layout_topBar_header_search_placeholder')"
      ...
      <div class="panel-search__tip">
        {{ getLocale('layout_topBar_header_search_tips_1') }}
        <span class="panel-search__key">{{ hotkey.open }}</span>
        {{ getLocale('layout_topBar_header_search_tips_2') }}
        <span class="panel-search__key">{{ hotkey.close }}</span>
        {{ getLocale('layout_topBar_header_search_tips_3') }}
      </div>
    </div>
    <div
...
export default {
  ...
  computed: {
    ...mapState('admin/search', ['hotkey', 'pool']),
    // 这份数据是展示在搜索面板下面的
    resultsList() {
